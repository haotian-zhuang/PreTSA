...
```{r eval=FALSE}
temporalTest(expr = expr, pseudotime = pseudotime, pseudotime_permute = pseudotime_permute, knot = F)
```

To reduce the computational time, users can ignore the pseudotime uncertainty, which is the default setting.
```{r}
testRes <- temporalTest(expr = expr, pseudotime = pseudotime, pseudotime_permute = NULL, knot = F)
```
